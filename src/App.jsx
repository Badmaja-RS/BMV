...
function App() {
  ...
  // load the initial track on first render
  if (!audio.src) {
  ...
  // ✅ Play/pause uses real audio state + awaits play()
...
const setTrack = useCallback(
  async (nextIndex, autoPlay) => {
    const audio = audioRef.current;
    if (!audio) return;

    // change selection immediately (UI updates)
    setCurrentSongIndex(nextIndex);

    // IMPORTANT: force audio to switch source immediately
    audio.pause();
    ...
    try {
      await audio.play();
      // isPlaying will also be set by onPlay, but setting here is ok too
      setIsPlaying(true);
    } catch (err) {
...
const setTrackAndMaybePlay = useCallback(
  async (index, autoPlay) => {
    const audio = audioRef.current;
    if (!audio) return;
    ...
    // set the new audio source directly (React is NOT controlling src anymore)
    audio.pause();
    ...
    try {
      await audio.play();
      // isPlaying will also sync via onPlay
    } catch (err) {
  ...
  // ✅ Next/Prev/Select only change track. Autoplay is handled by useEffect below.
  ...
  setTrack(nextIndex, wasPlaying);
}, [currentSongIndex, songs.length, isPlaying, setTrack]);
...
const handlePrevious = useCallback(() => {
  ...
  setTrack(prevIndex, wasPlaying);
}, [currentSongIndex, songs.length, isPlaying, setTrack]);

const handleSongSelect = useCallback(
  (index) => {
    setTrack(index, true); // selecting should play immediately
  },
  [setTrack],
);
  ...
  // ✅ Keep volume synced (don't use volume={...} on <audio>)
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    audio.volume = volume;
  ...
  // ✅ Key fix: when track changes, if currently playing, autoplay the new track
  // useEffect(() => {
  //   const audio = audioRef.current;
  //   if (!audio) return;
  ...
  //       console.log("Auto-play on track change failed:", err);
  //       setIsPlaying(false);
  //     });
  //   }
      ...
      >
        ...
        {showHoverPopup && (
          ...
          >
            ...
            >
              <button className="close-btn" onClick={closeHoverPopup} aria-label="Close">
                ✕
              </button>
        ...
        {showProsConsPopup && (
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.3, ease: "backOut" }}
            >
              <button className="close-btn" onClick={closeProsConsPopup} aria-label="Close">
                ✕
              </button>
